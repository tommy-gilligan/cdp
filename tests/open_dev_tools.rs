...
async fn open_dev_tools_test() {
    ...
    assert_eq!(
        client.page().enable().await,
        Ok(cdp::page::EnableReturn { __blank: () })
    );
    ...
    assert_eq!(
        ret,
        cdp::debugger::EnableReturn {
            debugger_id: "test id".to_owned()
        }
    );
    assert_eq!(
        client
            .debugger()
            .set_pause_on_exceptions("none".to_owned())
            .await,
        Ok(cdp::debugger::SetPauseOnExceptionsReturn { __blank: () })
    );
    assert_eq!(
        client.debugger().set_async_call_stack_depth(32).await,
        Ok(cdp::debugger::SetAsyncCallStackDepthReturn { __blank: () })
    );
    // Overlay.enable	{}	{}
    ...
    assert_eq!(
        client.profiler().enable().await,
        Ok(cdp::profiler::EnableReturn { __blank: () })
    );
    assert_eq!(
        client.log().enable().await,
        Ok(cdp::log::EnableReturn { __blank: () })
    );
    assert_eq!(
        client
            .log()
            .start_violations_report(vec![
                cdp::log::ViolationSetting {
                    name: "longTask".to_owned(),
                    threshold: 200.0
                },
                cdp::log::ViolationSetting {
                    name: "longLayout".to_owned(),
                    threshold: 30.0
                },
                cdp::log::ViolationSetting {
                    name: "blockedEvent".to_owned(),
                    threshold: 100.0
                },
                cdp::log::ViolationSetting {
                    name: "blockedParser".to_owned(),
                    threshold: -1.0
                },
                cdp::log::ViolationSetting {
                ...
                cdp::log::ViolationSetting {
                    name: "recurringHandler".to_owned(),
                    threshold: 50.0
                },
                cdp::log::ViolationSetting {
                    name: "discouragedAPIUse".to_owned(),
                    threshold: -1.0
                },
            ])
            .await,
    ...
    assert_eq!(
        client
            ...
            .await,
        Ok(cdp::runtime::AddBindingReturn { __blank: () })
    );
    ...
    let expected = cdp::dom::GetDocumentReturn {
        root: cdp::dom::Node {
            ...
            children: Some(vec![cdp::dom::Node {
                node_id: 2,
                parent_id: Some(1),
                backend_node_id: 2,
                node_type: 1,
            ...
            attributes: None,
            document_url: Some("about:blank".to_owned()),
            base_url: Some("about:blank".to_owned()),
            public_id: None,
            system_id: None,
            internal_subset: None,
